...
export default function UserFormModal({ show, handleClose, handleSave, selectedUser }) {
  ...
  useEffect(() => {
    if (selectedUser) {
      setFormData({ full_name: selectedUser.full_name || '', email: selectedUser.email || '', role: selectedUser.role || 'user', password: '' })
    } else {
    ...
    <Modal show={show} onHide={handleClose} centered>
      ...
      <Form onSubmit={onSubmit}>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" type="submit" style={{ background: '#7c3aed', border: 'none' }}>Guardar</Button>
        </Modal.Footer>
      </Form>
    </Modal>
  )
